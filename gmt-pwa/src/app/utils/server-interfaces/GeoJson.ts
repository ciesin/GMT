...
export const ALL_HEALTH_FACILITY_TYPES = [
  UNKNOWN_HEALTH_FACILITY_TYPE,
  FIXED_HEALTH_FACILITY_TYPE,
  OUTREACH_HEALTH_FACILITY_TYPE,
  MOBILE_HEALTH_FACILITY_TYPE,
] as const;
...
const ALL_HEALTH_FACILITY_TYPES_WITH_MC = [
  ...
  MOBILE_HEALTH_FACILITY_TYPE,
  'multiple_claimed',
] as const; //as const is needed to get type checking
...
export const ALL_CI_TYPES = ['generated', 'include', 'exclude'] as const;
...
export const ALL_HEALTH_FACILITY_LEVEL_OF_CARE = [
  //Unknown is needed because this cannot be null in the database, and for outreach this is not an editable field in the UI
  UNKNOWN,
  'Primary',
  'Secondary',
  'Tertiary',
...
export type HealthFacilityMaturityLevel =
  AllHealthFacilityMaturityLevel[number];
...
type AllHealthFacilityCatchmentStatus =
  typeof ALL_HEALTH_FACILITY_CATCHMENT_STATUS;

export type HealthFacilityCatchmentStatus =
  AllHealthFacilityCatchmentStatus[number];
...
export type HealthFacilityStaffPosition =
  AllHealthFacilityStaffPosition[number];
...
type AllHealthFacilityMeansOfTransport =
  typeof ALL_HEALTH_FACILITY_MEANS_OF_TRANSPORT;

export type HealthFacilityMeansOfTransport =
  AllHealthFacilityMeansOfTransport[number];

export const ROUTINE_IMMUNIZATION_SERVICE = 'Routine Immunization';
// .sort((a,b) => a.label.localeCompare(b.label))
export const ALL_HEALTH_FACILITY_SERVICES = [
  ROUTINE_IMMUNIZATION_SERVICE,
  'Antenatal Care',
  'CMAM',
...
export interface GeoJsonSettlementPartProperties {
  global_id: string;
  boundary_polygon: string;
...
export interface GeoJsonSettlementNameProperties {
  global_id: string;
  boundary_polygon: string;
  name: string;
  ...
  uninhabited_other_detail: string | null;
  problematic: Array<ProblematicOption>;
  is_primary: boolean;

  set_with_gps: boolean | null;
  comments: string;
...
export interface GeoJsonBoundaryProperties {
  ...
  num_fp_level_of_care?: number[];
  num_fp?: number;
  num_fp_ri?: number;
  num_fp_mp_ready?: number;
  ...
  num_set_problematic?: Array<number> | null;
  num_set_uninhabited?: Array<number> | null;

  num_set_mgn?: number | null;
  num_fp_mp_status?: number[];
  boundary_pop?: number | null;

  //[key: string]: PropertyValue};
}

export interface GeoJsonBoundaryWithIndicatorsProperties
  extends GeoJsonBoundaryProperties {
  boundary_pop: number | null;
  boundary_data_quality: number[];
  boundary_mp_status: number[];
  catchment_pop_fp?: number;
  catchment_pop_outreach?: number;
  catchment_pop_unclaimed?: number;
  catchment_pop_problematic?: number;
  num_boundary_participating?: number;
  num_fp?: number;
  num_fp_level_of_care?: number[];
  num_fp_mp_ready?: number;
  num_fp_mp_status: Array<number>;
  num_fp_mp_ready_participate: number | null;
  num_fp_participate: number | null;
  num_fp_ri?: number;
  num_no_geometry: number | null;
  num_no_hf: number | null;
  num_no_settlements: number | null;
  num_outreach: number | null;
  num_set_mgn: number | null;
  num_set_pop_diff: number | null;
  num_set_prob: number | null;
...
interface GeoJsonBoundaryEditedProperties {
  ...
  union?: boolean;
  comment?: null | string;
  is_edit?: boolean; // if true, then this is only temp edit suggestion saved to db and not the real boundary.  It can also be
  //the result of a resolved suggestion, as a sort of version history
}
...
export interface GeoJsonBoundaryWithIndicators extends GeoJsonBase {
  type: 'Feature';
  geometry: MultiPolygon;
  properties: GeoJsonBoundaryWithIndicatorsProperties;
}
...
export interface GeoJsonHealthFacilityProperties {
  global_id: string;
  boundary_polygon: string;
  name: string;
  synonyms: Array<string>;
  ...
  staff_positions_other: Array<string|null>;
  ...
  color?: string;
  index?: string;
  //for outreaches 0 of N
  //0 based
  childIndex?: number;
  numParentChildren?: number;
  created_date?: string;
...
export interface GeoJsonCatchmentProperties {
  global_id: string;
...
export interface GeoJsonCatchmentItem extends GeoJsonBase {
  ...
  geometry: Point;
  bbox?: Extent;
  properties: GeoJsonCatchmentProperties;
}
