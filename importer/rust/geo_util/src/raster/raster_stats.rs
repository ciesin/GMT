...
impl RasterStats {
    ...
    pub fn calc_x_round<I: IntAlias>(&self, x_coord: f64) -> I {
        I::from_f64(((x_coord - self.origin_x) / self.pixel_width).round()).unwrap()
    }
    pub fn calc_y_round<I: IntAlias>(&self, y_coord: f64) -> I {
        I::from_f64(((y_coord - self.origin_y) / self.pixel_height).round()).unwrap()
    }
    ...
    pub fn common_offsets(&self, rhs: &RasterStats) -> Offsets
    {
        ...
        let x1 : i32 = self.calc_x_round(rhs.origin_x);
        let x2 : i32 = rhs.calc_x_round(self.origin_x);

        //ensure no rounding problems
        assert_eq!(x1, -x2);

        let y1 : i32 = self.calc_y_round(rhs.origin_y);
        let y2 : i32 = rhs.calc_y_round(self.origin_y);
...
mod tests {
    ...
    fn test_coords() {
        ...
        assert_eq!(r1.calc_x_round::<i32>(4.0), 0);
        assert_eq!(r1.calc_x_round::<i32>(4.999), 1);
        assert_eq!(r1.calc_x_round::<i32>(5.0), 1);
