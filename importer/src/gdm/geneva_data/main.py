...
def main():
    ...
    python3.8 /src/gdm/geneva_data/main.py --config /config_files/docker_local_db.yml 7 --clean
...
def gdm_process(start_step, stop_step, util_cfg):
    ...
    step_list = [
        steps.step_import_raw_data,
        steps.step_prune_raw_data,
        steps.step_import_hf,
        
        steps.step_populate_boundary_table,
        steps.step_populate_hf_table,

        

        steps.step_populate_settlement_tables,

        # Can skip if already existst
        steps.step_create_europe_raster,
        steps.step_create_partitions,
        steps.step_calc_boundary_data,
